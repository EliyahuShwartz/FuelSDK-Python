...
class ET_GetRest(ET_Constructor):
    def __init__(self, auth_stub, endpoint, qs=None):
        ...
        headers = {'authorization': 'Bearer ' + auth_stub.authToken, 'user-agent': 'FuelSDK-Python-Extended-v1.3.4'}
...
class ET_PutRest(ET_Constructor):
    def __init__(self, auth_stub, endpoint, payload):
        auth_stub.refresh_token()

        headers = {'content-type': 'application/json', 'user-agent': 'FuelSDK-Python-v1.3.0', 'authorization': 'Bearer ' + auth_stub.authToken}
        r = requests.put(endpoint, data=json.dumps(payload), headers=headers)

        obj = super(ET_PutRest, self).__init__(r, True)
        return obj
...
class ET_CUDSupportRest(ET_GetSupportRest):
    ...
    def put(self):
        ...
        obj = ET_PutRest(self.auth_stub, completeURL, self.props)
        return obj
